...
@section('title', 'Create Session Feedback')
            ...
            <div class="px-6 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-slate-600 dark:text-slate-400 mt-1">Provide feedback for: {{ $session->title }}</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <div class="text-right">
                            <p class="text-sm font-medium text-slate-900 dark:text-white">{{ $session->club->club_name }}</p>
                            <p class="text-sm text-slate-600 dark:text-slate-400">Session Date: {{ $session->session_date->format('M d, Y') }}</p>
                        </div>
            ...
            <div class="max-w-4xl mx-auto">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-slate-700 dark:to-slate-700">
                        <h2 class="text-xl font-bold text-slate-900 dark:text-white">Feedback Form</h2>
                        <p class="text-slate-600 dark:text-slate-400 mt-1">Help improve teaching quality through constructive feedback</p>
                    </div>

                    <div class="p-8">
                        <form method="POST" action="{{ route('facilitator.feedback.store', $session) }}" class="space-y-8">
                            @csrf
                            ...
                            <div class="bg-slate-50 dark:bg-slate-700 rounded-xl p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <p class="text-slate-900 dark:text-white font-medium">{{ $session->teacher->name ?? 'No teacher assigned' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Session Time</label>
                                        <p class="text-slate-900 dark:text-white">{{ $session->session_date->format('M d, Y') }} at {{ $session->start_time }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Duration</label>
                                        <p class="text-slate-900 dark:text-white">{{ $session->duration }} minutes</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Students Attended</label>
                                        <p class="text-slate-900 dark:text-white">{{ $session->attendance_records->count() }} students</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400">How well was the content presented and explained?</p>
                                    <div class="flex items-center space-x-2">
                                        @for($i = 1; $i <= 5; $i++)
                                            <input type="radio" name="content_delivery_rating" value="{{ $i }}" id="content_delivery_{{ $i }}" required
                                                   class="sr-only star-rating">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400">How well did the teacher engage and interact with students?</p>
                                    <div class="flex items-center space-x-2">
                                        @for($i = 1; $i <= 5; $i++)
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400">How well was the session organized and managed?</p>
                                    <div class="flex items-center space-x-2">
                                        @for($i = 1; $i <= 5; $i++)
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400">How well was the teacher prepared for the session?</p>
                                    <div class="flex items-center space-x-2">
                                        @for($i = 1; $i <= 5; $i++)
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400">Overall assessment of the session</p>
                                    <div class="flex items-center space-x-2">
                                        @for($i = 1; $i <= 5; $i++)
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <label class="flex items-center space-x-3 p-4 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                                        <input type="radio" name="feedback_type" value="positive" required class="text-green-500 focus:ring-green-500">
                                        <div class="flex items-center space-x-2">
                                    ...
                                    <label class="flex items-center space-x-3 p-4 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                                        <input type="radio" name="feedback_type" value="constructive" required class="text-blue-500 focus:ring-blue-500">
                                        <div class="flex items-center space-x-2">
                                    ...
                                    <label class="flex items-center space-x-3 p-4 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                                        <input type="radio" name="feedback_type" value="critical" required class="text-red-500 focus:ring-red-500">
                                        <div class="flex items-center space-x-2">
                                    ...
                                    <label class="flex items-center space-x-3 p-4 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                                        <input type="radio" name="feedback_type" value="mixed" required class="text-yellow-500 focus:ring-yellow-500">
                                        <div class="flex items-center space-x-2">
                            ...
                            <div class="space-y-3">
                                ...
                                <p class="text-sm text-slate-600 dark:text-slate-400">Provide specific, constructive feedback about the session</p>
                                <textarea name="content" rows="6" required
                                          ...
                                          placeholder="Describe what went well, what could be improved, and specific suggestions for the teacher..."></textarea>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <p class="text-sm text-slate-600 dark:text-slate-400">Specific actionable recommendations</p>
                            ...
                            <div class="flex justify-end space-x-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                                ...
                                <button type="submit" 
                                        class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl">
                                    Submit Feedback
                                </button>
                            </div>
                        </form>
