                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-slate-600 dark:text-slate-400 mt-1">Update resource information and settings</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-4xl mx-auto">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-slate-700 dark:to-slate-700">
                        <h2 class="text-xl font-bold text-slate-900 dark:text-white">Update Resource Information</h2>
                        <p class="text-slate-600 dark:text-slate-400 mt-1">Modify the details for {{ $resource->title }}</p>
                    </div>
                        ...
                        <form method="POST" action="{{ route('admin.resources.update', $resource) }}" enctype="multipart/form-data" class="space-y-8">
                            ...
                            <div class="space-y-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <select name="club_id" required
                                            ...
                                            @foreach($clubs as $club)
                                                <option value="{{ $club->id }}" {{ old('club_id', $resource->club_id) == $club->id ? 'selected' : '' }}>{{ $club->club_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea name="description" rows="4" required
                                              class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md"
                                              placeholder="Describe the resource content and learning objectives">{{ old('description', $resource->description) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        ...
                                        <select name="attachment_type" id="resource-type" required onchange="toggleInputFields()"
                                            ...
                                            <option value="video" {{ old('attachment_type', $resource->attachment_type) == 'video' ? 'selected' : '' }}>Video Content</option>
                                            <option value="document" {{ old('attachment_type', $resource->attachment_type) == 'document' ? 'selected' : '' }}>Document (PDF, Word, PowerPoint)</option>
                                            <option value="image" {{ old('attachment_type', $resource->attachment_type) == 'image' ? 'selected' : '' }}>Image/Gallery</option>
                                            <option value="link" {{ old('attachment_type', $resource->attachment_type) == 'link' ? 'selected' : '' }}>External Link/Website</option>
                                            <option value="audio" {{ old('attachment_type', $resource->attachment_type) == 'audio' ? 'selected' : '' }}>Audio/Podcast</option>
                                            <option value="code" {{ old('attachment_type', $resource->attachment_type) == 'code' ? 'selected' : '' }}>Code Repository/Project</option>
                                            <option value="quiz" {{ old('attachment_type', $resource->attachment_type) == 'quiz' ? 'selected' : '' }}>Interactive Quiz</option>
                                            <option value="assignment" {{ old('attachment_type', $resource->attachment_type) == 'assignment' ? 'selected' : '' }}>Assignment Template</option>
                                            <option value="other" {{ old('attachment_type', $resource->attachment_type) == 'other' ? 'selected' : '' }}>Other</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <select name="visibility" required
                                            ...
                                            <option value="all" {{ old('visibility', $resource->visibility) == 'all' ? 'selected' : '' }}>All Users</option>
                                            <option value="teachers_only" {{ old('visibility', $resource->visibility) == 'teachers_only' ? 'selected' : '' }}>Teachers Only</option>
                                            <option value="students_only" {{ old('visibility', $resource->visibility) == 'students_only' ? 'selected' : '' }}>Students Only</option>
                                            <option value="private" {{ old('visibility', $resource->visibility) == 'private' ? 'selected' : '' }}>Private (Creator Only)</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div id="input-fields-container" class="space-y-4">
                                    <!-- Video URL Input -->
                                    <div id="video-url-input" class="space-y-2 {{ old('attachment_type', $resource->attachment_type) == 'video' ? '' : 'hidden' }}">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            Video URL <span class="text-red-500">*</span>
                                        </label>
                                        <div class="relative">
                                            <input type="url" name="video_url" value="{{ old('video_url', $resource->video_url) }}" placeholder="https://youtube.com/watch?v=... or https://vimeo.com/..."
                                                   class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                <svg class="h-5 w-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <!-- External Link Input -->
                                    <div id="external-link-input" class="space-y-2 {{ old('attachment_type', $resource->attachment_type) == 'link' ? '' : 'hidden' }}">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            External Link <span class="text-red-500">*</span>
                                        </label>
                                        <div class="relative">
                                            <input type="url" name="external_url" value="{{ old('external_url', $resource->external_url) }}" placeholder="https://example.com"
                                                   class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                <svg class="h-5 w-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <input type="text" name="link_title" value="{{ old('link_title', $resource->link_title) }}" placeholder="Link title (optional)"
                                               class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                    </div>

                                    <!-- Code Repository Input -->
                                    <div id="code-repo-input" class="space-y-2 {{ old('attachment_type', $resource->attachment_type) == 'code' ? '' : 'hidden' }}">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            Code Repository <span class="text-red-500">*</span>
                                        </label>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <input type="url" name="code_url" value="{{ old('code_url', $resource->code_url) }}" placeholder="https://github.com/user/repo or https://gitlab.com/user/repo"
                                                   class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                            <input type="text" name="code_branch" value="{{ old('code_branch', $resource->code_branch) }}" placeholder="Branch (optional, defaults to main)"
                                                   class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                        </div>
                                    ...
                                    <!-- Audio URL Input -->
                                    <div id="audio-url-input" class="space-y-2 {{ old('attachment_type', $resource->attachment_type) == 'audio' ? '' : 'hidden' }}">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            Audio URL <span class="text-red-500">*</span>
                                        </label>
                                        <div class="relative">
                                            <input type="url" name="audio_url" value="{{ old('audio_url', $resource->audio_url) }}" placeholder="https://soundcloud.com/... or direct audio URL"
                                                   class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                <svg class="h-5 w-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div id="file-upload-input" class="space-y-2 {{ in_array(old('attachment_type', $resource->attachment_type), ['document', 'image', 'quiz', 'assignment', 'other']) ? '' : 'hidden' }}">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            {{ $resource->attachment_url && in_array($resource->attachment_type, ['document', 'image', 'quiz', 'assignment', 'other']) ? 'Replace File' : 'Upload File' }}
                                        </label>
                                        
                                        @if($resource->attachment_url && in_array($resource->attachment_type, ['document', 'image', 'quiz', 'assignment', 'other']))
                                            <div class="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-600 mb-4">
                                                <div class="flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        <div class="font-medium text-slate-900 dark:text-white">{{ basename($resource->attachment_url) }}</div>
                                                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ ucfirst($resource->attachment_type) }} file</div>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="tags" value="{{ old('tags', $resource->tags) }}" placeholder="Enter tags separated by commas (e.g., beginner, python, tutorial)"
                                           class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
